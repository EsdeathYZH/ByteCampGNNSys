...
service GraphServices {
    ...
    // 获取指定点特征
    NodeFeature GetNodeFeature(1:NodeId node_id, 2:list<FeatureType> feature_id)
    // 获取指定点邻居
    Neighbor GetNodeNeighbors(1:NodeId node_id, 2:EdgeType edge_type)
    // 指定Author ID，拉取其P类型(边类型）的一跳邻居并返回特征（P为参数)
    list<IDFeaturePair> GetNeighborsWithFeature(1:NodeId node_id, 2:EdgeType neighbor_type, 3:list<FeatureType> feature_types)
    // 指定点类型（类型为参数）......带权随机采样
