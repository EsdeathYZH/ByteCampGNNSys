...
typedef i64 FeatureId
typedef i32 FeatureType
...
service GraphServices {
    ...
    // 获取指定点特征
    NodeFeature GetNodeFeature(1:NodeId node_id, 2:FeatureId feature_id)

    // 指定Author ID，拉取其P类型(边类型）的一跳邻居并返回特征（P为参数)
    list<IDFeaturePair> GetNeighborsWithFeature(1:NodeId node_id, 2:NodeType neighbor_type, 3:list<FeatureType> feature_types)

    // 指定点类型（类型为参数）......带权随机采样
    list<IDNeighborPair> SampleNeighbor(1:i32 batch_size, 2:NodeType node_type, 3:NodeType neighbor_type, 4:i32 sample_num)
